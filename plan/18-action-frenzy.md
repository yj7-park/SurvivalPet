...
1. **행동 수치(Action)** — 세 번째 생존 지표, 무위 상태에서 감소
2. **행동 수치 회복** — 다양한 활동 수행 시 회복
3. **반복 패널티** — 같은 활동 반복 시 회복량 점감
4. **광란 모드(Frenzy)** — 행동 수치 0 도달 시 30초간 조작 불가·자동 공격
5. **HUD 표시** — 행동 수치 게이지 + 광란 카운트다운
...
function getActionRecovery(base: number, combo: ActionCombo): number {
  // 다른 행동을 사이에 끼우면 콤보 리셋 (30초 이상 경과도 리셋)
...
재진입 방지:
- 광란 종료 후 5초 쿨다운 동안 행동 수치가 0이 되어도 광란 미진입
- 쿨다운 중 행동 수치는 계속 자연 감소하나 바닥은 1로 고정
...
기존 허기·피로 게이지와 동일한 스타일로 행동 게이지 추가:
...
광란 진행 중 저장:
- 광란 상태는 저장하지 않음 (불러오기 시 행동 수치 = 25로 초기화)
...
| 파일 | 변경 내용 |
|------|---------|
| `src/systems/ActionSystem.ts` | 신규: 행동 수치·광란 전체 관리 |
...
| `src/ui/HUD.ts` | 행동 수치 게이지 추가, 광란 카운트다운 오버레이 |
...
- 행동 수치는 멀티플레이어 모드에서 로컬 계산 (서버 동기화 안 함 — 클라이언트 전용)
